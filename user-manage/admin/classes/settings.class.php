...
class Settings extends Generic {
		...
		if (!empty($this->options['profile-field_section'])) :
			foreach($this->options['profile-field_section'] as $key => $value) :
				...
				$params = array(
					...
					':order'  => $this->options['profile-field_order'][$key],
					':id'      => $key
				);

				if ( $stmt->rowCount() < 1 )
					parent::query("INSERT INTO `login_profile_fields` (`id`, `section`, `type`, `label`, `public`, `signup`, `order`) VALUES (:id, :section, :type, :label, :public, :signup, :order);", $params);
				else
					parent::query("UPDATE `login_profile_fields` SET `section` = :section, `type` = :type, `label` = :label, `public` = :public, `signup` = :signup, `order` = :order WHERE `id` = :id", $params);
	...
	public function profile_fields() {
		...
		//echo "<pre>";print_r($stmt->fetch(PDO::FETCH_ASSOC));exit;
		while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) :
			...
			?><tr class="profile-field-row">
                ...
                <td><input type="text" value="<?php echo $row['order']; ?>" name="profile-field_order[<?php echo $i; ?>]" placeholder="<?php _e('Order'); ?>" class="form-control input-medium"></td>
			</tr><?php
			$i++;
		endwhile;
