...
class Generic extends Connect {
	...
	public function denyAccessLogs() {
		...
		return ( ($this->getOption('profile-timestamps-admin-enable') && !in_array(1, $_SESSION['simulation']['user_level'])) || !$this->getOption('profile-timestamps-enable') );
	...
	public function guestOnly() {
		...
		if(!empty($_SESSION['simulation']['username'])) {
			$this->error =	"
	...
	public function valid_token($token) {
		...
		if ($_SESSION['simulation']['token'] != $token)
			return false;
