...
class Check extends Generic {
	...
	private function isGuest($forceLogin) {
		...
		if ( !$forceLogin )
			return empty( $_SESSION['simulation']['user_id'] );

		if ( empty($_SESSION['simulation']['user_id']) ) :
			...
			$_SESSION['simulation']['referer'] = $_SERVER['REQUEST_URI'];
	...
	private function isActivated() {
		...
		if ( !empty($_SESSION['simulation']['activate']) ) :
	...
	private function isRestricted() {
		...
		if ( !empty($_SESSION['simulation']['restricted']) || !empty($_SESSION['simulation']['level_disabled']) ) :
	...
	private function forcePwUpdate() {
		...
		if ( !empty($_SESSION['simulation']['forcePwUpdate']) && basename($_SERVER['PHP_SELF']) != 'profile.php') :
	...
	public function protectPage($level) {
		...
		*/
		...
		if( ! @array_intersect($level, $_SESSION['simulation']['user_level']) && $level != array('*') )
			$this->deny_access();
	...
	public function protectThis($level) {
		...
		*/
		...
		if ( empty( $_SESSION['simulation']['user_id'] ) && $level == array('*') )
			return false;
