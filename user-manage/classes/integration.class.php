...
class Jigowatt_integration extends Generic {
	...
	private function retrieveInfo() {
		...
		$params = array( ':user_id' => $_SESSION['simulation']['user_id'] );
	...
	public function link_account( $link, $login = false ) {
		...
		try{
			...
			if ( !empty( $provider_user_profile ) ) {
				$_SESSION['simulation'][$link . 'Misc'] = (array) $provider_user_profile;
				$_SESSION['simulation'][$link] = $provider_user_profile->identifier;
			}

			/** See if the link was successful. */
			if ( !empty( $_SESSION['simulation'][$link] ) ) {
	...
	private function connect( $link ) {
		...
		$params = array(
			...
			':session_link' => $_SESSION['simulation'][$link]
		);
	...
	private function unlink( $provider ) {
		...
		unset( $_SESSION['simulation'][$provider] );
