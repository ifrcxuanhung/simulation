...
class Login extends jigowatt_integration {
	...
	function __construct() {
		...
		if ( parent::getOption('signin-redirect-referrer-enable') )
			$_SESSION['simulation']['referer'] = (!empty($_SESSION['simulation']['referer'])) ? $_SESSION['simulation']['referer'] : 'home.php';
		else
			$_SESSION['simulation']['referer'] = parent::getOption('signin-redirect-url');
		...
		foreach (parent::$socialLogin as $provider) :
			if (!empty($_SESSION['simulation'][$provider])) {
	...
	private function social_login($provider) {
		...
		$params = array( ':session' => $_SESSION['simulation'][$provider] );
		...
		if ($stmt->rowCount() > 0) {
			...
			unset(
				$_SESSION['simulation']['ot'],
				$_SESSION['simulation']['ots'],
				$_SESSION['simulation'][$provider]
			);
	...
	public function login() {
		...
		if ( parent::getOption('user-activation-enable') ) :
			...
			if ($count > 0) $_SESSION['simulation']['activate'] = 1;

		endif;

		// Save if user is restricted
		if ( !empty($this->result['restricted']) ) $_SESSION['simulation']['restricted'] = 1;
		...
		if (parent::getOption('pw-encrypt-force-enable')) :
			...
			if (strlen($this->result['password']) == 32 && $type == 'SHA256')
				$_SESSION['simulation']['forcePwUpdate'] = 1;
		...
		$_SESSION['simulation']['user_level'] = $user_level;

		$_SESSION['simulation']['email'] = $this->result['email'];

		$_SESSION['simulation']['gravatar'] = parent::get_gravatar($this->result['email'], true, 26);
		...
		if ( !empty($disRow['level_disabled']) ) $_SESSION['simulation']['level_disabled'] = 1;
		...
		$_SESSION['simulation']['username'] = $this->result[$username];

		// User ID of the logging in user
		$_SESSION['simulation']['user_id'] = $this->result['user_id'];
		//$_SESSION['simulation']['level_level'] = $this->result['level_level'];

		if ( empty($redirect) ) $redirect = $_SESSION['simulation']['referer'];
