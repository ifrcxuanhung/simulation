...
class Activate extends Generic {
	...
	function __construct() {
		...
		if(isset($_SESSION['simulation']['username']))
			$this->user = $_SESSION['simulation']['username'];
	...
	private function getKey() {
		...
		if ($stmt->rowCount() < 1) {
			$this->error = '<div class="alert alert-danger">' . _('Your activation link is incorrect.') . '</div>
					  ...
					  <p>' . sprintf(_('Go to the <a href="http://simulation.vndmi.com"> homepage</a>'), 'home.php') . '</p>';
			return false;
		}
		...
		if(!empty($_SESSION['simulation']['activate'])) unset($_SESSION['simulation']['activate']);
	...
	private function signedIn() {
		...
		if ($stmt->rowCount() < 1) {
			unset($_SESSION['simulation']['activate']);
