define([
    ...
    'backbone'
        ...
        var cleanarticleView = Backbone.View.extend({
            el: $(".main-container"),
            initialize: function() {
            ...
            events: {
                 "click a.show-modal": "actionShowModal",
				  "click a.update-modal": "actionUpdateModal",
			...
			actionSaveModal: function(event){
				...
				$("#check_modal").submit(function(e) {                       
                        ...
                        $.ajax({
                            url: $base_url + 'cleanarticle/update_modal',
                            type: 'POST',
			...
			actionRunQuery: function(event){
               ...
               $.ajax({
                      type : "POST",
                      url : $base_url + 'cleanarticle/runQuery',
 ...
 //                console.log(table.find('tr.active').children('.checkboxes').value());
			...
			actionDeleteField: function(event) {
				...
				var $this = $(event.currentTarget);
                $.ajax({
					url: $base_url + 'cleanarticle/delete_row',
					type: 'POST',
			...
			actionUpdateModal: function(event) {
                var $this = $(event.currentTarget);
                var type = $this.attr("type-modal");
				var param = $this.attr("keys_value");
...
//						});
            ...
            actionShowModal: function(event) {
               ...
               if(dataArr.length > 0){  
					$.ajax({
						url: $base_url + "cleanarticle/show_modal",
						type: "POST",
						data : {dataArr:dataArr},
						async: false,
						success: function(response) {
                   ...
                   var grid = new Datatable();
                        grid.init({
                            ...
                            dataTable: {
                                ...
                                "pageLength": 10,
                                "ajax": {
                                    "url": $base_url + "cleanarticle/loadcleanarticle/?name_table=" +name_table+"&category="+category+'&'+value_filter,
                                    "type": "POST"
                                },
