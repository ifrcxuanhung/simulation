	...
	//            }
				function getvalueCHART(){
					return $.ajax({
						url: $base_url + "ajax/getChart4",
						type: "POST",
						data: {code:code},
						async: false
					});
				}
				data = jQuery.parseJSON(getvalueCHART().responseText);
				
				//data1 = jQuery.parseJSON(getvalueCHART().responseText);
				//if (data1.length > 0) data1 = data1.perform.slice(1);
				//console.log(data);
				function getData() {
					//var data = [];
					//var res = [];
					...
					$("#chart_4").bind("plothover", function(event, pos, item) {
						...
						if (item) {
							if (previousPoint != item.dataIndex) {
								...
								showTooltip(item.pageX, item.pageY, "Close = " + y);
							}
						} else {
							$("#tooltip").remove();
							previousPoint = null;
	...
	//                        }
					...
					var plot = $.plot($("#chart_4"), [getData()], options);
