...
class Ion_auth_model extends CI_Model {
     ...
     * @author Mathew
     * */
    public function register($username, $password, $email, $additional_data = array(), $levels = array()) {
        ...
        if (!empty($levels)) {
            ...
            foreach ($levels as $level) {
                $this->add_to_user_info($level, $id);
            }
        }

        /*//add to default group if not already set
        $default_level = $this->where('lv_name', $this->config->item('default_level', 'ion_auth'))->group()->row();
        if ((isset($default_level->id) && !isset($levels)) || (empty($levels) && !in_array($default_level->id, $levels))) {
            $this->add_to_user_info($default_level->id, $id);
        }*/
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select($this->identity_column . ', username, first_name, email, id, password, active, last_login, services')
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                $session_data = array(
                    ...
                    'email' => $user->email,
                   // 'avatar' => $user->avatar,
                    'services' => $user->services, // 'services' => unserialize($user->services),
     ...
     * @author Ben Edmunds
     * */
    public function users($levels = NULL) {
        ...
        if (isset($levels)) {
            ...
            if (isset($levels) && !empty($levels)) {
                $this->db->distinct();
                $this->db->join(
                        $this->tables['users_info'], $this->tables['users_info'] . '.id_user = ' . $this->tables['users'] . '.id', 'inner'
                );

                $this->db->where_in($this->tables['users_info'] . '.user_level', $levels);
            }
        }
...
/**
     ...
     * @author Ben Edmunds
     * */
    public function get_users_level($id = FALSE) {
        $this->trigger_events('get_users_level');

        //if no id was passed use the current users id
        $id || $id = $this->session->userdata('user_id');

        return $this->db->select($this->tables['users_level'] . '.' . $this->join['users_level'] . ' as id, ' .$this->tables['users_level'] .'.lv_name, ' . $this->tables['users_level'] . '.info')
        				->join($this->tables['users_info'], $this->tables['users_info'] . '.'.$this->join['users_info'] . '=' . $this->tables['users_level'] . '.'.$this->join['users_level'])
                        ->where($this->tables['users_info'] . '.id_user', $id)                        
                        ->get($this->tables['users_level']);
    }
     ...
     * @author Ben Edmunds
     * */
    public function add_to_user_info($user_level, $user_id = false) {
        $this->trigger_events('add_to_user_info');

        //if no id was passed use the current users id
        $user_id || $user_id = $this->session->userdata('user_id');

        return $this->db->insert($this->tables['users_info'], array($this->join['users_info'] => (int) $user_level, 'id_user' => (int) $user_id));
    }
    ...
    public function update($id, array $data, $group_id) {
        ...
        $data_level = array('user_level' => $user_level);
        $this->db->update('user_info', $data_level, array('id_user' => $user->id));
    ...
    public function delete_user($id) {
       ...
       // $this->remove_from_group(NULL, $id);
        ...
        $this->db->delete($this->tables['users_info'], array('id_user' => $id));
