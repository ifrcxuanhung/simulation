...
class Ifrc_articles_new extends Welcome{
	protected $var_image ='';
    public function __construct() {
        parent::__construct();
		$this->load->model('Ifrc_articles_new_model', 'ifrc_articles_new');
    ...
    public function index($table ='ifrc_articles', $category = "") {
		...
		$table_sys = "ifrc_article";
		
		$headers = $this->table->get_headers("ifrc_articles_new");
		...
		foreach ($headers as $key => $value) {
            if(($value['type_search'] == 1)) {
				...
				if((strpos(strtolower($value['type']),'list')!==false)){
					$headers[$key]['filter'] = $this->table->append_select_new(strtolower($value['field']),'ifrc_articles','',$value_filter!='' ? $value_filter  : ($value['field']=='category' ? $category :'' ));
