...
class Table_model extends CI_Model {
    ...
    function append_input($field = '', $value_filter='') {
	 ...
	 function append_input_title($field = '', $value_filter='') {
        ...
        if($field == '') {
			...
			if($value_filter!=''){
				if($field=='title'){
           		 ...
           		 return '<input type="text" id="'.strtolower($field).'" name="'.strtolower($field).'" placeholder="Search..." class="form-control form-filter input-sm" autocomplete="off" value="'.$value.'"/>';
				}
			}
			else{
	 ...
	 function append_input_clean_order($field = '', $value_filter='') {
		
        if($field == '') {
            return '';
        } else {
			if($value_filter!=''){
				if($field=='clean_order'){
					
					if(isset($this->session->userdata['clean_order'])){
						$value = $this->session->userdata['clean_order'];	
					}
					else{ 
						$value = $value_filter;
					}
	 ...
	 function append_input_html_description($field = '', $value_filter='') {
		  ...
		  if($field == '') {
			...
			if($value_filter!=''){
				if($field=='html_description'){
					...
					if(isset($this->session->userdata['html_description'])){
						$value = $this->session->userdata['html_description'];	
					}
					else{ 
						$value = $value_filter;
					}
				
           		 return '<input type="text" id="html_'.strtolower($field).'" name="html_'.strtolower($field).'" placeholder="Search..." class="form-control form-filter input-sm" autocomplete="off" value="'.$value.'"/>';
				}	
			}
			else{
	 ...
	 function append_input_html_long_description($field = '', $value_filter='') {
		
       		  if($field == '') {
				return '';
			} else {
				if($value_filter!=''){
					if($field=='html_long_description'){
						
						if(isset($this->session->userdata['html_long_description'])){
							$value = $this->session->userdata['html_long_description'];	
						}
						else{ 
							$value = $value_filter;
						}
    ...
    function append_select($field = '',$arr='', $format_n='', $value_filter='') {
	 ...
	 function append_select_vnxindex($field = '',$arr='', $format_n='', $value_filter='') {
        if($field == '') {
            return '';
        } else {
            $html = '<select id="'.$field.'" name="'.$field.'" class="form-control form-filter input-sm" autocomplete="off">';
	 ...
	 function append_select_new($field = '',$arr='', $format_n='', $value_filter='') {
    ...
    function append_image_editon($image = '', $field='') {
		...
		$html .='<img id="img" alt="" src="'. (isset($image)&&$image!="" ? $base_url.$image : $base_url."assets/images/no-image.jpg") .'" class="img_upload_'.$field.'" style="max-height: 100px;" >';
		$html .='</div>';
		$html .='<div id="file">';
		$html .='<span class="btn btn-icon-only blue btn-file upload_image_multi" id="multi_'.$field.'">';
		$html .='<span class="fileinput-new">';
		$html .='<i class="fa fa-plus"></i> </span>';
		$html .='<span class="fileinput-exists"><i class="fa fa-edit"></i> </span>';
		$html .='<input name="'.$field.'" id="'.$field.'" class="avatar_'.$field.'" value="'. (isset($image)&&$image!="" ? $image : "assets/images/no-image.jpg") .'" >'; // type="file" class="file_upload"
		$html .='</span>';
		$html .='<a href="javascript:;" class="btn btn-icon-only red fileinput-exists delete-img" data-dismiss="fileinput">
