...
if (!function_exists('format_custom')) {
	function format_custom($val){
	
	$find_format = strpos($val,'.');
	$find_comma = strpos($val,',');
	...
	$result = $get_decimal.",'.','".$get_comma."'";
	echo $result;
			
	}
}

if (!function_exists('trans_table')) {
	function trans_table($code){
	
		$lang = $_SESSION['curent_language']['code'];
		$CI = & get_instance();
        $CI->load->model('welcome/Translate_model', 'translate');
        if (isset($CI->translate)) {
            $get_lang = $CI->translate->get_lang($lang,$code);
		}
		echo $get_lang;
...
if (!function_exists('translate')) {
    ...
    function translate($word, $return = FALSE) {
        $CI = & get_instance();
        $CI->load->model('welcome/Translate_model', 'translate');
        if (isset($CI->translate)) {
            $trans = $CI->translate->get_tran_col($word);
            if ($return == FALSE) {
                echo $trans;
            } else {
                return $trans;
...
if (!function_exists('get_trans')) {
    ...
    function get_trans($word, $lang, $return = FALSE) {
        $CI = & get_instance();
        $CI->load->model('welcome/Translate_model', 'translate');
        if (isset($CI->translate)) {
            $trans = $CI->translate->get_trans($word,$lang);
            if ($return == FALSE) {
                echo $trans;
            } else {
                return $trans;
...
function image_thumb_w($image_path, $width = "", $fixed = 1) {
    // Get the CodeIgniter super object
    $CI = & get_instance();
    ...
    $image_thumb = $thumb_dir . '/' . $width . '_' . $filename . '.' . $ext;
    list($w, $h) = getimagesize($image_path);	
	$height=0;
	if($w>0)
    $height = ($h * $width) / $w;
...
if( !function_exists('ceiling') )
{
    function ceiling($number, $significance = 1)
    {
		if(is_numeric($number) && is_numeric($significance)){
			$up = (ceil($number/$significance)*$significance);
			$down = (floor($number/$significance)*$significance);
			if(abs($up-$number) <= abs($down-$number)) return $up;
			else return $down;
		}
		else {
			return false;
