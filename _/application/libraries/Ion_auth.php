...
class Ion_auth
{
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $additional_data = array(), $level = array()) //need to test email activation
	{
		...
		if (!$email_activation)
		{
			$id = $this->ion_auth_model->register($username, $password, $email, $additional_data, $level);
			if ($id !== FALSE)
			{
	...
	public function is_admin($id=false)
	{
		...
		return $this->in_level($admin_group, $id);
	}
...
/**
	 ...
	 * @author Phil Sturgeon
	 **/
	public function in_level($check_level, $id=false)
	{
		$this->ion_auth_model->trigger_events('in_level');

		$users_levels = $this->ion_auth_model->get_users_level($id)->result();
		$levels = array();
		foreach ($users_levels as $level)
		{
			$levels[] = $level->lv_name;
		}

		if (is_array($check_level))
		{
			foreach($check_level as $key => $value)
			{
