/**
*
* @license Guriddo jqGridMobile JS - v1.2.0 - 2015-08-10
...
/**
 * @license jqGridMobile  1.2   - jQuery Grid for mobile devices
 * Copyright (c) 2012 - 2015, Tony Tomov, tony@trirand.com
 * 
...
$.extend($.jgrid,{
	...
	styleUI : {
		jQueryMobile : {
			...
			base : {
				entrieBox : "ui-responsive ui-corner-all", // entrie div  incl everthing
				viewBox : "", // view diw
			...
			celledit : { // not used yet
				inputClass : ""
			}, 
			inlinedit : { //  not used yet
				inputClass : "",
				icon_edit_nav : "",
				icon_add_nav : "",
				icon_save_nav : "",
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			refreshContent : true,
			dataTheme : ""
		}, $.jgrid.defaults , $.jgrid.mobile, pin );
		if (localData !== undefined) {
			p.data = localData;
			pin.data = localData;
		}
		var ts= this, scrollgrid, grid={
			headers:[],
			cols:[],
			footers: [],
		...
		data_page = $(this).closest("div:jqmData(role='page')");
		if(!ts.p.dataTheme) {
			ts.p.dataTheme = "a";
			if( data_page.length ) {
		...
		addMulti = function(rowid, pos, irow, checked, uiclass){
			var	v = "<input data-role=\"none\" role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+ts.p.id+"_"+rowid+"\" "+uiclass+" name=\"jqg_"+ts.p.id+"_"+rowid+"\"" + (checked ? "checked=\"checked\"" : "")+"/>",
		...
		setPager = function (pgid, tp){
			...
			var sep = "<td class='ui-pg-button'></td>",
			pginp = "",
			pgl="<table style='table-layout:auto;height:100%;' class='ui-pg-table ui-common-table'><tbody><tr>",
			...
			$("#"+$.jgrid.jqID(pgid) )
			.append("<div id='"+pgcnt+"' class='ui-pager-control' role='group' data-role='footer' data-theme='"+ts.p.dataTheme+"'><table class='ui-pg-table ui-common-table' style='width:100%;table-layout:fixed;height:100%;'><tbody><tr><td id='"+lft+"' align='left'></td><td id='"+cent+"' align='center' style='white-space:pre;'></td><td id='"+rgt+"' align='right'></td></tr></tbody></table></div>")
			.attr({"dir" : "ltr","data-role":"footer"}); //explicit setting
			var strsb = "";
			if(ts.p.rowList.length >0){
				strsb ="<tr>";
				strsb += "<td><label for=\"select-choice-1\"></label>"+$.jgrid.getRegional(ts, 'defaults.recordPage')+"</td>";
				strsb += "<td><select data-mini='true' name='select-choice-1' id='select-choice-1' "+getstyle(stylemodule, 'pgSelectBox', false, 'ui-pg-selbox')+" role=\"listbox\">";
				var strnm;
				for(i=0;i<ts.p.rowList.length;i++){
					...
					strsb +="<option role=\"option\" value=\""+strnm[0]+"\""+(( intNum(ts.p.rowNum,0) === intNum(strnm[0],0))?" selected=\"selected\"":"")+">"+strnm[1]+"</option>";
				}
				strsb +="</select></td></tr>";
			}
			if(dir==="rtl") { pgl += str; }
			if(ts.p.pginput===true) { 
				...
				pginp= "<td dir='"+dir+"'"+'style="text-align:center">' +'<a ' + getstyle(stylemodule, 'icon_popup',false,'pagersettings') +' href="#" data-rel="popup" data-transition="pop" style="margin-top:0;margin-bottom:0;"></a></td>';
			}
			if(ts.p.pgbuttons===true) {
				var po=["first"+tp,"prev"+tp, "next"+tp,"last"+tp], btc=getstyle(stylemodule, 'pgButtonBox', true, 'ui-pg-button'); 
				if(dir==="rtl") { po.reverse(); }
				/*
				pgl += "<td id='"+po[0]+"' class='"+btc+"' title='"+($.jgrid.getRegional(ts,"defaults.pgfirst",ts.p.pgfirst) || "") +"'" + "><span " + getstyle(stylemodule, 'icon_first', false, iconbase)+"></span></td>";
				pgl += "<td id='"+po[1]+"' class='"+btc+"'  title='"+($.jgrid.getRegional(ts,"defaults.pgprev",ts.p.pgprev) || "") +"'" +"><span " + getstyle(stylemodule, 'icon_prev', false, iconbase)+"></span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='"+btc+"' title='"+($.jgrid.getRegional(ts,"defaults.pgnext",ts.p.pgnext) || "") +"'" +"><span " + getstyle(stylemodule, 'icon_next',false, iconbase)+"></span></td>";
				pgl += "<td id='"+po[3]+"' class='"+btc+"' title='"+($.jgrid.getRegional(ts,"defaults.pglast",ts.p.pglast) || "") +"'" +"><span " + getstyle(stylemodule, 'icon_end',false, iconbase)+"></span></td>";
				*/
				pgl += "<td id='"+po[1]+"' style='text-align:center'><a href='#' " + getstyle(stylemodule, 'icon_prev')+" style='margin-top:0;margin-bottom:0;'></a></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' style='text-align:center'><a href='#' " + getstyle(stylemodule, 'icon_next')+" style='margin-top:0;margin-bottom:0'></a></td>";
			...
			if(ts.p.pgbuttons===true) {
				...
				*/
				$("#prev"+$.jgrid.jqID(tp)+", #next"+$.jgrid.jqID(tp)).click( function() {
					if ($(this).hasClass(disabled)) {
						return false;
					}
			...
			if(ts.p.pginput===true) {
				...
				'<div data-role="popup"  data-position-to="#gbox_'+ts.p.id+'" data-overlay-theme="a" data-theme="'+ts.p.dataTheme+'" style="min-width:200px;" class="ui-corner-all" id="pgpop_'+pgid+'">'+
					'<div data-role="header" data-theme="'+ts.p.dataTheme+'" class="ui-corner-top">'+
						'<h1>'+$.jgrid.getRegional(ts, "defaults.pagerCaption")+'</h1>'+
					'</div>'+
					'<table style="width:100%"><tr>'+
					'<td style="width:28%"><label for="slider-1">'+($.jgrid.getRegional(ts, "defaults.pageText") || "" )+'</label></td>'+
					'<td "width:72%"><input id="slider-1" name="slider-1" type="range" class="ui-pg-input" type="text" size="4" maxlength="7" value="0" role="textbox" style="width:29px;height:30px" min="1" max="5"/></td>'+
					'</tr>'+
					strsb+
					'</table>'+
					'<div style="text-align:right">'+
						'<a href="#" data-role="button" data-inline="true" data-rel="back" data-mini="true" data-theme="'+ts.p.dataTheme+'">Cancel</a>'+
						'<a class="setpage" href="#" data-role="button" data-inline="true" data-mini="true" data-transition="flow" data-theme="'+ts.p.dataTheme+'">Update</a>'+
					'</div>'+
				'</div>');
		...
		multiSort = function(iCol, obj ) {
			...
			$(".ui-grid-ico-sort",selTh).addClass(disabled);
			...
			if( so ) {
				...
				$(".ui-ico-"+so,selTh).removeClass(disabled);
		...
		sortData = function (index, idxcol,reload,sor, obj){
			...
			if(ts.p.multiSort) {
				...
				tmpicon = $(previousSelectedTh).find(".ui-grid-ico-sort");
				...
				if(ts.p.frozenColumns) {
					tmpicon = ts.grid.fhDiv.find(".ui-grid-ico-sort");
					tmpicon.addClass(disabled);
					if(usehide) { tmpicon.css("display","none"); }
					ts.grid.fhDiv.find("th").attr("aria-selected","false");
				}
				tmpicon = $(newSelectedTh).find(".ui-ico-"+ts.p.sortorder);
		...
		imgs += "<a href='#' sort='asc'  class='ui-grid-ico-sort ui-ico-asc"+iac+" ui-sort-"+dir+" "+disabled+" " + iconbase + " " + getstyle(stylemodule, 'icon_asc', true)+ "'" + sort + "></a>";
		imgs += "<a href='#' sort='desc' class='ui-grid-ico-sort ui-ico-desc"+idc+" ui-sort-"+dir+" "+disabled+" " + iconbase + " " + getstyle(stylemodule, 'icon_desc', true)+"'" + sort + "></a></span>";
		if(ts.p.multiSort) {
			if(ts.p.sortname ) {
				...
				if(ts.p.multiSort) {
					if(ts.p.viewsortcols[0]) {
						...
						if(ts.p.colModel[j].lso){ 
							$("div .ui-ico-"+ts.p.colModel[j].lso,this).removeClass(disabled).css("display","");
						}
					...
					if(ts.p.viewsortcols[0]) {
						$("div span.s-ico",this).show(); 
						if(j===ts.p.lastsort){ 
							$("div .ui-ico-"+ts.p.sortorder,this).removeClass(disabled).css("display","");
						}
		...
		$(grid.hDiv).attr({"data-theme":ts.p.dataTheme, "data-role":"header"}).append(hb);
		$(hb).append(hTable);
		hTable = null;
		...
		$(grid.bDiv)
			.attr({"data-role" : "content", "data-theme" :ts.p.dataTheme,"id":"bdiv_"+ts.p.id })
			//.append($('<div class="iScroller"></div>')
			.append($('<div style="overflow:auto;display:table;"></div>')
			.append((ts.p.scrollPaging || ts.p.scroll) ? '<div id="pullDown_'+ts.p.id+'" class="pullDown"><span class="pullDownIcon"></span><span class="pullDownLabel">' + $.jgrid.getRegional(ts, "defaults.scrollPulldown") + '</span></div>':'')
			.append(this)
			.append((ts.p.scrollPaging || ts.p.scroll) ? '<div id="pullUp_'+ts.p.id+'" class=pullUp"><span class="pullUpIcon"></span><span class="pullUpLabel">' + $.jgrid.getRegional(ts, "defaults.scrollPullup") + '</span></div>':''))
			.addClass( getstyle(stylemodule,'bodyDiv', true, "ui-jqgrid-bdiv"))
		...
		if(ts.p.multikey){
			if( $.jgrid.msie) {
				...
				$(grid.bDiv).bind("vmousedown",function(){return false;});
			}
		}
		if(hg) { // hidden grid
			$(grid.bDiv).hide();
		...
		var arf = ts.p.hidegrid===true ? $("<div  class='ui-jqgrid-titlebar-close HeaderButton "+cornerall+"' title='"+($.jgrid.getRegional(ts, "defaults.showhide", ts.p.showhide) || "")+"'" + " ></div>").hover(
			function(){ arf.addClass(hover);},
			function() {arf.removeClass(hover);})
		.append("<a class='ui-jqgrid-headlink " + icoo +"'></a>").css((dir==="rtl"?"left":"right"),"0px") : "";
		$(grid.cDiv)
			.append(arf).append("<span class='ui-jqgrid-title'>"+ts.p.caption+"</span>")
		...
		if( ts.p.toolbar[0] ) {
			...
			if(ts.p.toolbar[1]==="both") {
				...
				$(grid.uDiv).addClass( tbstyle + " ui-userdata-top").attr("id","t_"+this.id).insertBefore(grid.hDiv);
				$(grid.ubDiv).addClass( tbstyle + " ui-userdata-bottom").attr("id","tb_"+this.id).insertAfter(grid.hDiv);
		...
		if(ts.p.toppager) {
			...
			grid.topDiv = $("<div data-role='header' data-theme='"+ts.p.dataTheme+"' id='"+ts.p.toppager+"'></div>")[0];
		...
		if(ts.p.footerrow) {
			grid.sDiv = $("<div class='ui-jqgrid-sdiv ui-content'  data-theme='"+ts.p.dataTheme+"'></div>")[0];
			hb = $("<div class='ui-jqgrid-hbox"+(dir==="rtl"?"-rtl":"")+"'></div>");
				...
				$(".ui-jqgrid-titlebar-close",grid.cDiv).click( function(e){
					...
					if(ts.p.gridstate === 'visible') {
						$(elems,"#gbox_"+$.jgrid.jqID(ts.p.id)).slideUp("fast", function() {
							counter--;
							if (counter === 0) {
								$("a",self).removeClass(icoo).addClass(icoc);
						...
						$(elems,"#gbox_"+$.jgrid.jqID(ts.p.id)).slideDown("fast", function() {
							counter--;
							if (counter === 0) {
								$("a",self).removeClass(icoc).addClass(icoo);
		...
		data_page.bind('pageshow',function(){
			...
			if(ts.p.scrollPaging || ts.p.scroll){
				...
				myiScroll.bounce = true;
				myiScroll.useTransition = true;
				myiScroll.onRefresh = function() {
					if (pullDownEl.className.match('loading1')) {
						pullDownEl.className = '';
						pullDownEl.querySelector('.pullDownLabel').innerHTML = $.jgrid.getRegional(ts, "defaults.scrollPulldown");
					} else if (pullUpEl.className.match('loading1')) {
						pullUpEl.className = '';
						pullUpEl.querySelector('.pullUpLabel').innerHTML = $.jgrid.getRegional(ts, "defaults.scrollPullup");
					}
				...
				myiScroll.onScrollMove = function () {
					...
					if (this.y > 5 && !pullDownEl.className.match('gridflip')) {
						...
						pullDownEl.querySelector('.pullDownLabel').innerHTML = $.jgrid.getRegional(ts, "defaults.scrollRefresh");
						this.minScrollY = 0;
						...
						pullUpEl.querySelector('.pullUpLabel').innerHTML = $.jgrid.getRegional(ts, "defaults.scrollRefresh");
				...
				myiScroll.onScrollEnd = function () {
					if (pullDownEl.className.match('gridflip')) {
						...
						pullDownEl.querySelector('.pullDownLabel').innerHTML = $.jgrid.getRegional(ts, "defaults.loadtext", ts.p.loadtext);
						if(ts.p.page >1 ) {
							ts.p.page--;
							populate();
						} else {
							pullDownEl.className ='';
							pullDownEl.querySelector('.pullDownLabel').innerHTML = $.jgrid.getRegional(ts, "defaults.nomorerecs", ts.p.nomorerecs);
						}
						setTimeout(function(){scrollgrid.scrollTo(0,-50, 100);}, 100);
					} else if (pullUpEl.className.match('gridflip')) {
						pullUpEl.className = 'loading1';
						pullUpEl.querySelector('.pullUpLabel').innerHTML = $.jgrid.getRegional(ts, "defaults.loadtext", ts.p.loadtext);
						var yu = this.y, nmr=false;
						if(ts.p.lastpage >= ts.p.page+1) {
							ts.p.page++;
							populate();
						} else {
							pullUpEl.className ='';
							pullUpEl.querySelector('.pullUpLabel').innerHTML = $.jgrid.getRegional(ts, "defaults.nomorerecs", ts.p.nomorerecs);
							nmr = true;
						}
						if(!ts.p.scroll && !nmr) {
							setTimeout(function(){scrollgrid.scrollTo(0,-50, 100);}, 100);
						} else {
							setTimeout(function(){scrollgrid.scrollTo(0, yu + 50, 100);}, 100);
						}
					}
				};
			}
			...
			setTimeout(function(){ 
				...
				if(ts.p.scrollPaging || ts.p.scroll){
					scrollgrid.scrollTo(0,-50, 100);
				}
				//ts.scrollgrid = scrollgrid;
			},0);
		...
		return this.each(function(){
			...
			*/
			switch(p.loadtype) {
				...
				default:
					try {
						$.mobile.loading( p.method , { html:  p.htmlcontent });
					} catch(lme){}
			}
			/*
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				$t.grid.fhDiv = $('<div style="position:absolute;left:0px;top:'+top+'px;height:'+hth+'px;" class="frozen-div ' + hd +'"></div>');
		...
		*/
		...
		this.createTableForGroup = function(group, parentgroup) {
			...
			if(this.p.groupButton) {
				inputAddSubgroup = $("<input type='button' value='+ {}' title='Add subgroup' class='add-group ui-mini" + common.button + "'/>");
			...
			var inputAddRule = $("<input type='button' value='+' title='Add rule' class='add-rule ui-add ui-mini" + common.button + "'/>"), cm;
			...
			if (parentgroup !== null) { // ignore the first group
				var inputDeleteGroup = $("<input type='button' value='-' title='Delete group' class='delete-group ui-mini " + common.button + "'/>");
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			if(this.p.ruleButtons === true) {
			var ruleDeleteInput = $("<input type='button' value='-' title='Delete rule' class='delete-rule ui-del ui-mini " + common.button + "'/>");
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			buildRuleMenu = function( elem, left, top ){
				...
				var str = '<ul id="sopt_menu" data-theme="'+$t.p.dataTheme+'" class="ui-search-menu modal-content" data-role="listview" data-inset="true" style="left:'+left+'px;top:'+top+'px;z-index:100000">',
				...
				for ( i = 0 ; i < options.sopt.length; i++) {
					...
					if(ina !== -1) {
						selclass = (selected === (p.odata && p.odata[ina].oper)) ? common.highlight : "";
				...
				str += "</ul>";
				$('body').append(str);
				$("#sopt_menu").addClass("ui-menu " + classes.menu_widget).listview();
				$("#sopt_menu > li > a")
				.click(function() {
			...
			$.each($t.p.colModel,function(ci){
				...
				if(this.search){
					if(p.searchOperators) {
						...
						select = "<a title='"+st+"' style='padding-right: 0.5em;' soper='"+so+"' class='soptclass ui-btn ui-corner-all ui-btn-inline ui-mini' colname='"+this.name+"'>"+sot+"</a>";
					}
					...
					if(soptions.clearSearch) {
						...
						$("td:eq(2)",stbl).append("<a title='"+csv+"' class='clearsearchclass "+classes.icon_clear+"'>"+p.resetIcon+"</a>");
					} else {
						$("td:eq(2)", stbl).hide();
					}
					if(this.surl) {
	...
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				bS  ="<a id='"+fid+"_search' class='" +classes.icon_search + "'>"+p.Find+"</a>",
				bC  ="<a id='"+fid+"_reset' class='"+classes.icon_reset + "'>"+p.Reset+"</a>",
				bQ = "", tmpl="", colnm, found = false, bt, cmi=-1;
				if(p.showQuery) {
					bQ ="<a id='"+fid+"_query' class='"+classes.icon_query + "'>Query</a>";
				}
				if(!p.columns.length) {
				...
				fid = $.jgrid.jqID( fid);
				var afr = p.afterRedraw;
				if(!$.isFunction(afr) ) {
					afr = function() {};
				}
				p.afterRedraw = function() {
					afr();
					$( "#" + IDs.themodal ).enhanceWithin();
				};
				$("#"+fid).jqFilter({
					columns: columns,
				...
				*/
				var mcnt = $('<div id="'+IDs.themodal+'" data-role="popup" data-mini="true">'+
				'<a href="#" data-rel="back" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right">Close</a>'+
				'<div id="'+IDs.modalhead+'"  data-role="header"> <h1>'+p.caption+'</h1></div>'+
				'<div id="'+IDs.modalcontent+'"  data-role="content" class="ui-jqdialog-content ui-content"></div>'+
				'</div>');
				$("#"+IDs.modalcontent, mcnt).append(fil);
				$.mobile.activePage.append(mcnt).trigger( "pagecreate" );
				...
				setTimeout(function() {
					$( "#" + IDs.themodal ).popup();
					//$( "#" + IDs.themodal ).enhanceWithin();//.popup("refresh");
					$( "#" + IDs.themodal ).popup( "open", {positionTo: p.gbox} );
					$( "#" + IDs.themodal ).enhanceWithin();
				},100);
				$( document ).on( "popupafterclose", "#" + IDs.themodal, function() {
					$( this ).remove();
				});
				//showFilter($("#"+fid));
				/*
...
$.jgrid.extend({
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			bp = rtlb ? "nData" : "pData",
			bn = rtlb ? "pData" : "nData";
			var bP = "<a href='#' id='"+bp+"' class='" + styles.icon_prev+ "'></a>",
			bN = "<a href='#' id='"+bn+"' class='" + styles.icon_next+ "'></a>",
			bS  ="<a href='#' id='sData' class='" + styles.icon_save + "'>"+p.bSubmit+"</a>",
			bC  ="<a href='#' id='cData' class='" + styles.icon_cancel + "'>"+p.bCancel+"</a>";
			var bt = "<table style='height:auto' class='EditTable ui-common-table' id='"+frmtborg+"_2'><tbody></tr><tr id='Act_Buttons'><td class='navButton'>"+(rtlb ? bN+bP : bP+bN)+"</td><td class='EditButton'>"+bS+bC+"</td></tr>";
			...
			tms = $("<div></div>").append(frm).append(bt);
			//data_page = $(this).closest("div:jqmData(role='page')");
			var mcnt = $('<div id="'+IDs.themodal+'" data-role="popup" data-dismissable="false" data-overlay-theme="a" data-theme="'+$t.p.dataTheme+'" style="width:'+dw+'">'+
			'<div id="'+IDs.modalhead+'"  data-role="header" class="popupheader" > <b>'+p.caption+'</b></div>'+
			'<div id="'+IDs.modalcontent+'"  data-role="content" class="ui-jqdialog-content"></div>'+
			'</div>');
			$("#"+IDs.modalcontent, mcnt).append(tms);
			$.mobile.activePage.append(mcnt).trigger( "pagecreate" );
			...
			*/
			if(rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
				bS  ="<a href='#' id='sNew' data-role='button' data-inline='true' data-mini='true' style='z-index:1002'>"+p.bYes+"</a>";
				bN  ="<a href='#' id='nNew' data-role='button' data-inline='true' data-mini='true' style='z-index:1002'>"+p.bNo+"</a>";
				bC  ="<a href='#' id='cNew' data-role='button' data-inline='true' data-mini='true' style='z-index:1002'>"+p.bExit+"</a>";
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			bn = rtlb ? "pData" : "nData",
				// buttons at footer
			bP = "<a href='#' id='"+bp+"' class='" + styles.icon_prev+ "'></a>",
			bN = "<a href='#' id='"+bn+"' class='" + styles.icon_next+ "'></span></a>",
			bC  ="<a id='cData' class ='"+ styles.icon_close+"'>"+p.bClose+"</a>";
			if(maxRows >  0) {
				var sd=[];
				$.each($(tbl)[0].rows,function(i,r){
					sd[i] = r;
			...
			*/
			var tms = $("<div></div>").append(frm).append("<table border='0' class='EditTable' id='"+frmtb+"_2'><tbody><tr id='Act_Buttons'><td class='navButton' width='"+p.labelswidth+"'>"+(rtlb ? bN+bP : bP+bN)+"</td><td class='EditButton'>"+bC+"</td></tr></tbody></table>"),
			mcnt = $('<div id="'+IDs.themodal+'" data-role="popup" data-overlay-theme="a" data-dismissible="false" style="width:'+p.width+'px;" data-theme="'+$t.p.dataTheme+'" >'+
			'<div id="'+IDs.modalhead+'"  data-role="header" class="popupheader"> <b>'+p.caption+'</b></div>'+
			'<div id="'+IDs.modalcontent+'"  data-role="main" class="ui-content ui-jqdialog-content"></div>'+
			'</div>');
			$("#"+IDs.modalcontent, mcnt).append(tms);
			$.mobile.activePage.append(mcnt).trigger( "pagecreate" );
	...
	delGridRow : function(rowids,p) {
		...
		return this.each(function(){
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				var bS  ="<a href='#' id='dData' class ='ui-btn ui-btn-inline ui-mini ui-corner-all'>"+p.bSubmit+"</a>",
				bC  ="<a href='#' id='eData' class ='ui-btn ui-btn-inline ui-mini ui-corner-all'>"+p.bCancel+"</a>";

				tbl += "<table class='EditTable ui-common-table' id='"+dtbl+"_2'><tbody><tr><td class='DelButton EditButton'>"+bS+"&#160;"+bC+"</td></tr></tbody></table>";
				p.gbox = "#gbox_"+$.jgrid.jqID(gID);
				//$.jgrid.createModal(IDs,tbl, rp_ge[$t.p.id] ,"#gview_"+$.jgrid.jqID($t.p.id),$("#gview_"+$.jgrid.jqID($t.p.id))[0]);
				
				var	tms = '<div id="'+IDs.themodal+'" data-role="popup" data-theme="'+$t.p.dataTheme+'" data-dismissible="false" style="width:auto">'+
				'<div id="'+IDs.modalhead+'"  data-role="header" style="text-align:center"> <h3>'+p.caption+'</h3></div>'+
				'<div id="'+IDs.modalcontent+'"  data-role="main" class="ui-content ui-jqdialog-content">'+tbl+'</div></div>';

				$.mobile.activePage.append( tms ).trigger( "pagecreate" );
	...
	navGrid : function (elem, p, pEdit, pAdd, pDel, pSearch, pView) {
		...
		return this.each(function() {
			...
			if ($("#"+alertIDs.themodal)[0] === undefined) {
				$("#gbox_"+$t.p.id).after('<div data-theme="'+$t.p.dataTheme+'" data-role="popup" id="'+alertIDs.themodal+'" data-overlay-theme="a" style="max-width:'+o.alertwidth+'px;" class="ui-corner-all">'+
			    '<div data-role="header" class="ui-corner-top"><h1>'+o.alertcap+'</h1></div>'+
                '<div data-role="content" class="ui-corner-bottom ui-content" id="alid">'+o.alerttext+"</div>"+
				'<div style="text-align:center;"><a href="#" data-theme="'+$t.p.dataTheme+'" data-role="button" data-inline="true" data-mini="true" data-rel="back" >OK</a></div>'+
                '</div>');
			}
			...
			for(i = 0; i<clone; i++) {
				...
				select = "<div class='ui-pg-div'>";
				select += '<select class="ui-nav-select" name="select-nav" id="select-nav" data-mini="true" data-native-menu="false" data-inline="true" data-shadow="false" data-theme="'+$t.p.dataTheme+'">';
				select += '<option value="choose-one" data-placeholder="true">'+ ( o.selectcaption || 'Actions')+'</option>';
				tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
				//$(tbd).append(</div>");
				
				if (o.add) {
					pAdd = pAdd || {};
					select += '<option id="'+(pAdd.id || "add_"+elemids)+'" value="add">'+o.addtitle+'</option>';
				}
				if (o.edit) {
					pEdit = pEdit || {};
					select += '<option id="'+(pEdit.id || "edit_"+elemids)+'" value="edit">'+o.edittitle+'</option>';
				}
				if (o.view) {
					pView = pView || {};
					select += '<option id="'+(pView.id || "view_"+elemids)+'" value="view">'+o.viewtitle+'</option>';
				}
				if (o.del) {
					pDel = pDel || {};
					select += '<option id="'+(pDel.id || "del_"+elemids)+'" value="del">'+o.deltitle+'</option>';
				}
				
				if (o.search) {
					pSearch = pSearch || {};
					select += '<option id="'+(pSearch.id || "search_"+elemids)+'" value="search">'+o.searchtitle+'</option>';
				}
				if (o.refresh) {
					select += '<option id="'+("refresh_"+elemids)+'" value="refresh">'+o.refreshtitle+'</option>';
				}
				select += "</select></div>";
				...
				.on( "change", function(event) { 
					var selitem = $(this).find('option:selected'), sr;
					if(!selitem.hasClass( commonstyle.disabled )) {	
						var aa = $(this).val();
						switch (aa) {
						case  'add' : 
				...
				setTimeout(function(){
					$("#select-nav-button").css({"margin-top":"-6px"});
				},100);
				navtbl =null;
				$t.p.navGrid = true;
			}
	...
	navButtonAdd : function (elem, p) {
		...
		return this.each(function() {
			...
			if (findnav) {
				...
				var rid = $.jgrid.randId(),
				tbd = $("<option value='"+rid+"'>"+p.caption+"</option>");
				if(p.id) {$(tbd).attr("id",p.id);}
				if(p.position==='first'){
					if(findnav.rows[0].cells.length ===0 ) {
						$(".ui-nav-select",findnav).append(tbd);
					} else {
						$(".ui-nav-select",findnav).prepend(tbd);
					}
				} else {
					$(".ui-nav-select",findnav).append(tbd);
				}
				var  pp  = $($t).data('navGrid');
				pp[rid] = p.onClickButton;
				$($t).data('navGrid', pp);
			}
		});
	},
...
addSubGridCell :function (pos,iRow) {
	...
	return "<td role=\"gridcell\" aria-describedby=\""+sid+"_subgrid\" class=\"ui-sgcollapsed sgcollapsed\" "+prp+"><a style='cursor:pointer;' class='ui-sghref "+ic+"'></a></td>";
},
				...
				$(ts.rows[i].cells[pos]).bind('click', function() {
					...
					if($(this).hasClass("sgcollapsed")) {
						...
						if(ts.p.subGridOptions.reloadOnExpand === true || ( ts.p.subGridOptions.reloadOnExpand === false && !$r.hasClass('ui-subgrid') ) ) {
							...
							$(tr).after( "<tr role='row' id='" + pID + "_" + _id + "_expandedContent" + "' class='ui-subgrid ui-sg-expanded'>"+atd+"<td class='" + common.content +" subgrid-cell'></td><td colspan='"+parseInt(ts.p.colNames.length-1-nhc,10)+"' class='" + common.content +" subgrid-data'><div id="+pID+"_"+_id+" class='tablediv'></div></td></tr>" );
						...
						$(this).html("<a style='cursor:pointer;' class='ui-sghref "+ts.p.subGridOptions.minusicon+"'></a>").removeClass("sgcollapsed").addClass("sgexpanded");
						...
						$(this).html("<a style='cursor:pointer;' class='ui-sghref "+ts.p.subGridOptions.plusicon+"'></a>").removeClass("sgexpanded").addClass("sgcollapsed");
...
//module begin
	...
	$.fn.fmatter.checkbox =function(cval, opts) {
		...
		randomid = "jqgridcheck"+ Math.floor(Math.random()*11);
