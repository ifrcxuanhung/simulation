...
class newsletter {
	...
	public function getMemberField($db){
		...
		$kq = qa($sql_member_field, $db);
		 foreach ($kq as $field) {
			$field_name .=  "{ label : '$field[field_name]',name: '$field[field_name]',width: 100},";
		 }	
		 return $field_name;
	}
...
//    }
 ...
 public function get_send($db, $send_id, $obj = "", $st = "") {
...

        {
            ...
            switch ($obj) {
                ...
                case "email":
                    ...
                    $order = " ORDER BY email $st ";

                    break;

                case "open":

                    $order = " ORDER BY open_time $st ";

                    break;

                case "unsub":

                    $order = " ORDER BY unsubscribe_date $st ";

                    break;

                case "boun":

                    $order = " ORDER BY bounce_time $st ";
        ...
        $sql = "SELECT * FROM `" . _DB_PREFIX . "newsletter_member` nm LEFT JOIN `" . _DB_PREFIX . "member` m USING (member_id) WHERE nm.send_id = '" . mysql_real_escape_string($send_id) . "' AND nm.status != 1 $order";
...
//status = 6 la truong hop dang gui email ma nhan Pause
        $sql = "SELECT * FROM `" . _DB_PREFIX . "send` WHERE `status` = 3 || `status` = 6 ORDER BY finish_time DESC";
    ...
    public function send_out_newsletter($db, $send_id, $member_id, $newsletter_id = false, $force = false) {
        ...
        if ($status) {
            ...
            $options = array(
             ...
             $send_email_status = $this->send_email($replace['to_email'], $replace['email_subject'], $newsletter_html, $replace['from_email'], $replace['from_name']);
			//echo "<pre>";print_r($send_email_status);exit;
